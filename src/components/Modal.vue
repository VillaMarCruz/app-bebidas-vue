<script setup lang="ts">
    ...
    const formatearIngredientes = () => {
        ...
        for (let i = 1; i <= 15; i++) {
            if (bebidas.recipe['strIngredient' + i.toString()]) {
                const ingrediente = bebidas.recipe['strIngredient' + i.toString()];
                const cantidad = bebidas.recipe['strMeasure' + i.toString()];
                        ...
                        <DialogPanel
                            ...
                            <div>
                                <div class="mt-3">
                                    ...
                                    <img :src="bebidas.recipe.strDrinkThumb" :alt="bebidas.recipe.strDrink"
                                        class="mx-auto w-96">
                                    <DialogTitle as="h3" class="text-gray-900 text-4xl font-extrabold my-5">
                                    ...
                                    <p class="text-lg text-gray-500">{{ bebidas.recipe.strInstructions }}</p>
                                </div>
                            </div>
                            <div class="mt-5 sm:mt-6 flex justify-between gap-4">
                              ...
                              <button type="button" class="w-full rounded bg-orange-600 p-3 font-bold uppercase text-white shadow hover:bg-orange-500" @click="favoritos.handleClickFavorito">{{modal.textButton}}</button>
                            </div>
                        </DialogPanel>
                    </TransitionChild>
                </div>
